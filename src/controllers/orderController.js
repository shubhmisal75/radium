...
const updateOrder = async (req, res) => {
    try {
        ...
        if (isOrderBelongsToUser["cancellable"] == false) {
            if (isOrderBelongsToUser['status'] == 'cancelled') {
                return res.status(400).send({ status: false, message: "Unable to cancel the order due to the non-cancellable policy." });
            }
            if (isOrderBelongsToUser['status'] == 'pending') {
                if (status) {
                    if (!(status == 'pending' || status == 'completed')) {
                        return res.status(400).send({ status: false, message: `This order is already in Non-cancellable status.` })
                    }
                }
